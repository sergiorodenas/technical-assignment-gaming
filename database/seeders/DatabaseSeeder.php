...
class DatabaseSeeder extends Seeder
{
    ...
    {
        $users = collect()->times(10000)->map(function(){
            return User::factory()->create();
        });
        
        $weapons = collect()->times(100)->map(function(){
            return Weapon::factory()->create();
        });

        // This takes around 20 minutes (10M rows, 1GB)
        ...
        // Generate user,weapon high scores grouping by two columns (takes around 10min on my laptop and could be optimized)
        ...
        // Generate user high scores
        HighScore::selectRaw('MAX(high_score) as high_score, user_id')->groupBy('user_id')->get()->each(function($result){
            User::find($result->user_id)->update(['high_score' => $result->high_score]);
        });
    }
}
